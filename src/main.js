    ...
    workersContainer.innerHTML+=`
     <div
                            ...
                            <div>
                                ...
                                <div id="post-${worker.id}" class="cursor-pointer"><svg width="22px" fill="#334155"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><!--!Font Awesome Free v7.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M320.4 64C302.7 64 288.4 78.3 288.4 96L288.4 128L128.4 128C110.7 128 96.4 142.3 96.4 160L96.4 224C96.4 241.7 110.7 256 128.4 256L288.4 256L288.4 320L135 320C130.8 320 126.7 321.7 123.7 324.7L75.7 372.7C69.5 378.9 69.5 389.1 75.7 395.3L123.7 443.3C126.7 446.3 130.8 448 135 448L288.4 448L288.4 544C288.4 561.7 302.7 576 320.4 576C338.1 576 352.4 561.7 352.4 544L352.4 448L512.4 448C530.1 448 544.4 433.7 544.4 416L544.4 352C544.4 334.3 530.1 320 512.4 320L352.4 320L352.4 256L505.8 256C510 256 514.1 254.3 517.1 251.3L565.1 203.3C571.3 197.1 571.3 186.9 565.1 180.7L517.1 132.7C514.1 129.7 510 128 505.8 128L352.4 128L352.4 96C352.4 78.3 338.1 64 320.4 64z"/></svg></div>
                                <div id="profile-${worker.id}" class="cursor-pointer"><svg width="22px" fill="#334155" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><!--!Font Awesome Free v7.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M384 64C366.3 64 352 78.3 352 96C352 113.7 366.3 128 384 128L466.7 128L265.3 329.4C252.8 341.9 252.8 362.2 265.3 374.7C277.8 387.2 298.1 387.2 310.6 374.7L512 173.3L512 256C512 273.7 526.3 288 544 288C561.7 288 576 273.7 576 256L576 96C576 78.3 561.7 64 544 64L384 64zM144 160C99.8 160 64 195.8 64 240L64 496C64 540.2 99.8 576 144 576L400 576C444.2 576 480 540.2 480 496L480 416C480 398.3 465.7 384 448 384C430.3 384 416 398.3 416 416L416 496C416 504.8 408.8 512 400 512L144 512C135.2 512 128 504.8 128 496L128 240C128 231.2 135.2 224 144 224L224 224C241.7 224 256 209.7 256 192C256 174.3 241.7 160 224 160L144 160z"/></svg></div>
                            </div>
                        </div>
...
<section id="prfl" class="relative animation bg-white w-full max-w-[95%]  rounded-xl overflow-hidden  ">
            ...
            <div class="sm:flex sm:flex-col items-center ">
                <img class="w-24 rounded-full relative  m-auto top-[-3rem] border-4 border-white " src=${el.image !== undefined ? el.image : "/images/profile.jpg"} alt="profile">
              <div class="flex flex-col gap-6  w-full py-6">
                <div class="flex items-center  ml-5  "><p class="w-[20%] text-sm font-bold text-gray-800">Full Name : </p> <p class=" w-full text-center text-gray-500 text-sm ">${el.firstname + " " + el.lastname}</p></div>
                <div class="flex items-center  ml-5 "><p class=" w-[20%] text-sm font-bold text-gray-800">Email : </p> <p class="w-full text-center  text-gray-500 text-sm ">${el.email}</p></div>
                <div class="flex items-center  ml-5 "><p class=" w-[20%] text-sm font-bold text-gray-800">Phone : </p> <p class="w-full text-center  text-gray-500 text-sm ">${el.number}</p></div>
                <div class="flex items-center  ml-5 "><p class=" w-[20%] text-sm font-bold text-gray-800">Job : </p> <p class="w-full text-center  text-gray-500 text-sm ">${el.job}</p></div>
                <div class="flex items-center  ml-5 "><p class=" w-[20%] text-sm font-bold text-gray-800">Post : </p> <p class="w-full text-center  text-gray-500 text-sm ">${el.role}</p></div>
                <div class="flex items-center  ml-5 "><p class=" w-[20%] text-sm font-bold text-gray-800">Company : </p> <p class="w-full text-center  text-gray-500 text-sm ">${el.company}</p></div>
                ...
                <button id="remove-${el.id}" class="bg-red-600 rounded-md h-8 w-20 text-gray-50 text-sm ml-auto">remove</button>
            </div>
        </section>
...
postS.innerHTML=`
        ...
        <div id="cf-${el.id}" class="${el.role === "Manager" || el.role === "Réceptionniste" ? "bg-white cursor-pointer " : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4">Salle de conférence</div>    
        <div id="rn-${el.id}" class="${el.role === "Réceptionniste" || el.role === "Manager" || el.role === "Nettoyage " ? "bg-white cursor-pointer" : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4" >Réception</div>    
        <div id="sr-${el.id}" class="${el.role === "Technicien IT" || el.role === "Manager" || el.role === "Nettoyage" ? "bg-white cursor-pointer" : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4" >Salle des serveurs</div>    
        <div id="sc-${el.id}" class="${el.role === "Agent de sécurité" || el.role === "Manager" || el.role === "Nettoyage" ? "bg-white cursor-pointer" : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4">Salle de sécurité</div>    
        <div id="pr-${el.id}" class="${el.role === "Manager" || el.role === "Nettoyage" ? "bg-white cursor-pointer" : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4" >Salle du personnel</div>    
        <div id="ar-${el.id}" class="${el.role === "Agent de sécurité" || el.role === "Manager" ? "bg-white cursor-pointer" : "bg-gray-300 cursor-not-allowed"} h-12 rounded-md flex items-center pl-4" >Salle d’archives</div>   
...
if(el.role === "Manager" || el.role === "Réceptionniste" ){
    const isExist = posts_conference.some((item) => item.id === el.id )
    if(!isExist){
        posts_conference.push(el)
        console.log(posts_conference);
        postsConference()   
    }
}
})
...
if(el.role === "Réceptionniste" || el.role === "Manager" || el.role === "Nettoyage "){
    const isExist = posts_reception.some((item) => item.id === el.id )
    if(!isExist){
...
posts_conference.forEach((el)=>{
    ...
    cfSpace.innerHTML+=`
     ...
     <div id="div-worker${el.id}" class="bg-white flex items-center gap-1 p-1 rounded-md  ">
                                <img src=${el.image !== undefined ? el.image : "/images/profile.jpg"} alt="profile"  class="rounded-full w-4 lg:w-8 ">
                                <div class="sm:w-16 lg:w-24">
                                    <p class="text-[.4rem] lg:text-[.6rem] text-gray-800 font-semibold">${el.firstname + " " + el.lastname}</p>
                                    <p class="text-gray-600 font-semibold text-[.35rem] lg:text-[.5rem]">${el.role}</p>
                                </div>
                            </div> 
